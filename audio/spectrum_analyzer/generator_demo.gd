...
func _process(delta: float) -> void:
	...
	if $Sweep.selected == 1 or $Sweep.selected == 2:
		var nval = $FrequencyHSlider.value + delta*1000*(0.5 if $Sweep.selected == 1 else 2.0)
		$FrequencyHSlider.value = $FrequencyHSlider.min_value if nval >= $FrequencyHSlider.max_value else nval
