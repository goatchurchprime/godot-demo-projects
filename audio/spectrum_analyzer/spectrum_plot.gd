...
func refresh_analyzer_instance():
	...
	spectrumanalyzer.fft_size = $HBox/FFTSize.selected
	spectrumanalyzer.buffer_length = int($HBox/BufferLength.get_item_text($HBox/BufferLength.selected))
	...
	var fft_size = [256, 512, 1024, 2048, 4096][spectrumanalyzer.fft_size];
	var mix_rate = AudioServer.get_mix_rate();
	var fft_count = (spectrumanalyzer.buffer_length / (float(fft_size) / mix_rate)) + 1;
